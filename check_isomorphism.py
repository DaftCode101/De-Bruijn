...
"""
    Math research code.
    Shows that key(n, k) is the inverse line graph of lay(n, k).

    Author: Benjamin Keefer
    Version: July 7th, 2024
"""
...
def show_graph():
    ...
    if k != 2:
        # print(len(G.edges()))
        # print(len(line_of_lay.edges()))
        print(nx.is_isomorphic(G, line_of_lay))
        display_graph(G)
        display_graph(line_of_lay)

    line_of_lay = nx.line_graph(G)
    return

# Sets up lay(n, k)
def setup(new_n : int, new_k : int):
    global n, k, sequence
    k = new_k
    n = new_n
    print(f"n: {n}, k: {k}")
...
"""
Merges redundant edges in lay (not used currently)
"""
def merge_lay(new_n : int, new_k : int) -> None:
    setup(new_n, new_k)
    global sequence

    match_word = ("0" * (n-1)) + f"{k-1}"
    replacement_word = f"{k-1}" + ("0" * (n-1))
    
    sequence.remove((match_word, replacement_word))
    merged_lay = []

    for edge in sequence:
        if edge[1] == match_word:
            merged_lay.append((edge[0], replacement_word))
        else:
            merged_lay.append(edge)
    sequence = merged_lay

    print("-" * 20)
    show_graph()

# Displays the given graph using networkx
...
"""
Checks if Key(Lay(n, k)) is the inverse line graph of Lay(n, k).
"""
def check_key_line_isomorphism(new_n : int, new_k : int):
    global sequence
    setup(new_n, new_k)
    ...
    lay_n_k = nx.DiGraph(sequence)
    # display_graph(lay_n_k)

    key()
    key_graph = nx.DiGraph(sequence)
    # display_graph(key_graph)

    line_key_graph = nx.line_graph(key_graph)
    # display_graph(line_key_graph)
    line_of_key_is_isomorphic = nx.is_isomorphic(lay_n_k, line_key_graph)
    if line_of_key_is_isomorphic:
        print(f"The line graph of Key({n}, {k}) is isomorphic to Lay({n}, {k})!")
    else:
        print(f"The line graph of Key({n}, {k}) is NOT isomorphic to Lay({n}, {k}).")
    print(line_of_key_is_isomorphic)
    assert(line_of_key_is_isomorphic)
