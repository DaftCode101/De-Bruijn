...
"""
    ...
    a specific (onion) layer of a de Bruijn graph as well as computing properties
    of that layer for research.
...
class debruijn:
    ...
    def val_star(s):
        n = len(s)
        return max([debruijn.val(s[i:] + s[:i]) for i in range(n)])
    def rotate_zeros(s):
        return s if s[0]!=0 else debruijn.rotate_zeros(s[1:]+s[:1])
    def a_dagger(s):
        return s[-1] != 0 and debruijn.val_star(s)==debruijn.val(debruijn.rotate_zeros(s))
    def shift(s):
        x = s[:-1]
        sigma = s[-1]
        if sigma < (k - 1) and debruijn.a_dagger(x + [sigma + 1]):
            return [sigma + 1] + x
        elif debruijn.a_dagger(x + [sigma]):
            return [0] + x
        else:
            return [sigma] + x
    ...
    def adjacency_lists():
        ...
        for edge in sequence:
            if(edge[0] in debruijn.adjacency):
                debruijn.adjacency[edge[0]].append(edge[1])
            else:
                debruijn.adjacency[edge[0]] = [edge[1]]
    ...
    def num_of_paths(start, end, print_paths) -> int:
        ...
        visited = {}
        for edge in list(debruijn.adjacency.keys()):
            visited[edge] = False
        ...
        def dfs(current_path, current_vertex, print_paths):
            ...
            for neighbor in debruijn.adjacency[current_vertex]:
    ...
    # # Supposed to be the opposite of the line_graph function, but currently
    # # creates innacurate reflexivity in vertices
    ...
    def find_hamiltonian_paths(lay_val):
        ...
        paths = debruijn.num_of_paths(start, end, True)

        debruijn.endTimer()
        print(debruijn.dTime())
        print("Number of hamiltonian paths: " + str(paths))
        print("Number of vertices: " + str(len(debruijn.adjacency)))
    ...
    # Generates the graph's adjacency matrix and does computations on it
    def adjacency_matrix():
        ...
        # the laplacian, num. of arborescences, and the eigenvalues of the graph
        ...
        sub_arrays = debruijn.produce_sub_arrays(laplacian)
        print_line(l)
    ...
    def run():
        ...
        debruijn.de_bruijn(k, n)

        # Updates sequence according to lay
        if(lay_bool == "y"):
            debruijn.lay(lay_val)

        # Constructs layer of the de Bruijn graph
        debruijn.load_edges()
        debruijn.remove_zero_edges()
        debruijn.unique()
        debruijn.adjacency_lists()
        debruijn.line_graph()

        # debruijn.find_hamiltonian_paths(lay_val) # O(v!)
        debruijn.graph_is_eulerian()
        
        debruijn.G.add_edges_from(sequence)
        debruijn.adjacency_matrix()
        debruijn.show_graph()
        return
